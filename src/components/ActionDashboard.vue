...
<div class="focus-grid"><article v-for="c in top" :key="c.id" class="card panel"><div class="tags"><span class="pill priority">{{c.priority}}</span><span class="pill">{{fitScore(c)}} 分</span></div><h3>{{c.name}}</h3><p>{{c.recommendedRoles.slice(0,2).join(' / ')}}</p><p class="meta">{{c.fitReason}}</p><RouterLink :to="'/companies/'+c.id">公司分析与招聘入口 →</RouterLink></article></div>
